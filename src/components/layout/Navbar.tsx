...
import fireIcon from '@/assets/images/Navbar/fire.svg'
import avatarImage from '@/assets/images/Navbar/Avatar.png'

const Navbar = () => {
  return (
    <div className="bg-solid h-[60px] w-full flex items-center justify-between">
        <div className="flex items-center justify-between px-4">
            <img src={lightLogo} alt="Logo" className="max-w-[180px] h-auto dark:hidden" />
            <img src={darkLogo} alt="Logo" className="max-w-[180px] h-auto hidden dark:block" />
        </div>


        <div className="flex items-center gap-4 px-4">
            <div className="flex items-center gap-2 bg-borderMuted rounded-full px-3 h-8 max-w-[66px]">
                <img src={fireIcon} alt="Fire" className="w-5 h-5" />
                <span className="font-medium text-gray-800 dark:text-gray-200">30</span>
            </div>
            {/* Notification Bell */}
            <button className="w-8 h-8 rounded-full border border-gray-300 dark:border-gray-600 flex items-center justify-center bg-transparent hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors text-textPrimary">
